...
| **Shape an idea before building** | “Use Incline to brainstorm a hand-controlled plane window for my portfolio. Help me decide how it should look and move; show small comparisons where useful, then turn the chosen direction into a plan.” | Reuses known context, asks focused questions about choices that affect the result, and separates your decisions from proposed defaults before planning assets and implementation. |
...
| **Explore a reference without a detailed brief** | “Use Incline. I like this website—show me a few ways its character could work in my app.” | The agent inspects the reference, proposes plausible reasons for its appeal, and renders distinct adaptations. You can simply pick what feels closer; its guesses stay provisional. |
| **Remember exploration and return to alternatives** | “Use Incline to explore this reference for my app. You can try different interpretations; keep track of the attempts so we can return to other directions if one doesn't work. Show me a few distinct samples at a time.” | Saves this exploration's hypotheses, versioned attempts and feedback links locally. Unsuccessful branches remain recoverable, and a rejection does not automatically establish why the design failed. |
| **Compare a new reference with the current identity** | “Use Incline to show a few samples: one where this reference's distinctive feature leads, one where my current design leads, and a combination if it works. Let me choose, remember my choice for this project, then build it.” | The agent renders comparable studies, keeps the original available, records the contextual choice and available snapshots, and checks the implementation against the chosen sample. |
...
For an underspecified idea, the agent first uses [Brainstorm](skills/incline/references/workflows/brainstorm.md) to establish a shared creative brief. Following the Superpowers approach, it understands the intended use, asks one consequential question at a time, compares meaningful approaches and reflects the choices back. Small visual or playable motion studies can resolve choices that are hard to describe. Clear briefs and specific edits skip repeated intake; delegated choices remain agent defaults, not learned preferences.

The agent then works backward from the agreed experience into stages: choose HTML/CSS/SVG or generated assets, establish shared composition and lighting, prototype uncertain mechanisms, and delegate independent work before integration. Each substantial stage carries its inputs, outputs, concrete steps and observable checks so another worker can act without guessing the intent. Saved recipes can support a single stage or an adaptable workflow. The main agent retains the overall design direction. Incline supplies planning guidance and dependency data; it does not run a separate autonomous scheduler.
...
Try: **“Use Incline. I like this website—show me a few ways its character could work in my app.”** You do not need to list every detail you like. The agent inspects the whole reference and proposes grounded explanations of its appeal; any notes you do give help focus the work. These explanations are hypotheses to explore, not established facts about your taste.
...
The experimental [visual-study workflow](skills/incline/references/visual-studies.md) makes consequential design choices visible before a larger build. Samples can explore different plausible interpretations of a reference. When a new feature competes with the current identity, the agent can show a reference-led sample, a sample led by the existing design, and a coherent combination where useful. You can pick parts, keep the original, or reject all the samples; explaining why is optional. A clear edit or an already chosen direction does not require another comparison.

Studies use the host agent's rendering tools, not a new screen in the collection UI; image generation is optional. The agent preserves versioned task artifacts, normally under `.incline/studies/`, and uses the existing feedback journal when recording is active or you ask it to remember the choice. Selecting a sample guides this project's implementation; it does not approve every property or establish a global taste. The workflow is implemented as skill guidance, with design-quality validation still pending. See the [research and trial protocol](docs/evaluations/reference-transfer-research.md).

[Recoverable exploration](skills/incline/references/exploration.md) adds immutable checkpoints under `.incline/studies/<id>/exploration/`, a compact resume view, and targeted checks of linked screenshots and feedback. The agent can consider a broad pool while showing a few samples at a time, then refine, park, return to, or combine interpretations. Recording must already be active or explicitly requested for this exploration; stopping it also stops new feedback capture through this layer. The existing journal, insights, profile and personal library keep their formats. The tools validate history and provenance; real design-quality evaluation remains separate.
