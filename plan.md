...
The first prompt-hub delivery implements source-attributed prompts, editable faceted tags, local retrieval, immutable copies/revisions, and separate execution, inspection, tester and user-review evidence. Project and explicitly selected personal stores remain separate. The lead agent plans production stages, chooses code or generated assets, and checks compatible outputs before integration; optional subagents use existing host tools. The local library UI and portable CLI expose entries without changing legacy collections. Optional Jev selection is an offline-preview/explicit-send advisory adapter for one recipe-inspection decision, with one-call consumption, strict validation, freshness checks and receipts. It does not execute recipes or establish design quality. See the [implementation plan](docs/superpowers/plans/2026-09-26-prompt-hub.md), [design spec](docs/superpowers/specs/2026-09-26-prompt-hub-design.md) and [offline evaluation with known limits](docs/superpowers/plans/2026-09-26-prompt-hub-evaluation.md). Real-project design and live selector comparisons remain the next validation step; worker routing, automatic recovery and an autonomous scheduler remain deferred.
