...
Proposed execution-focused extension: the [curated prompt hub and staged design production proposal](docs/superpowers/specs/2026-09-26-prompt-hub-design.md) adds source-attributed prompts, editable faceted tags, lightweight contextual retrieval, and revision-specific testing evidence. The lead agent works backward from an intended effect, chooses code or generated assets for each part, and plans stages with shared visual contracts and explicit dependencies. Independent stages can use bounded subagents through existing host tools; the lead agent checks integration and the assembled experience. Saved recipes can support individual stages or adaptable workflows. Inspired by the reviewed Keel harness, optional Jev routing starts with one bounded decision: select an eligible recipe ID for inspection or abstain. Local code handles exact tags, constraints, freshness, and fallbacks; decision receipts link the choice to subsequent results. Trial it in advisory mode against the ordinary retrieval path before allowing it to prioritise reads. Worker routing and recovery/check selection remain later candidates, while planning and aesthetic judgment stay with the lead agent and user evidence. This is a design proposal, not implemented functionality, and does not replace the real-project evaluations below.
...
Implemented extension: [Reference Understanding and Recoverable Exploration](docs/superpowers/plans/2026-09-22-reference-exploration.md) separates broad reference interpretation from recovery after an unsuccessful attempt. The project-local derivation graph now has immutable checkpoints, exact artifact/feedback links, compact resume views, and targeted integrity checks. Park failed branches without deleting them or interpreting sample rejection as proof against every underlying hypothesis. Evaluate understanding and branch-aware recovery separately against matched inputs and budgets, including the user's explanation burden. The existing journal, insights, and knowledge index stay in place. Runtime verification passes; real design benefit remains untested. See the [behavior report and validation limits](docs/evaluations/reference-exploration.md).
