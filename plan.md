...
Proposed execution-focused extension: the [curated prompt hub and component prompt authoring design](docs/superpowers/specs/2026-09-26-prompt-hub-design.md) adds source-attributed prompts, editable faceted tags, lightweight contextual retrieval, and revision-specific testing evidence. A focused authoring workflow adapts recipes for crucial components under one shared design direction, then checks the assembled result. Optional Jev routing would review selected textual compatibility and relevance; it would not establish visual quality or user preference. This is a design proposal, not implemented functionality, and does not replace the real-project evaluations below.
