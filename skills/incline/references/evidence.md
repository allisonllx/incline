...
`.incline/prompts/` contains separate source-preserving prompt revisions and run evidence. `.incline/prompt-settings.json` controls scoped personal prompt lookup and prompt run/decision recording. Follow [prompt operations](prompts.md); these settings do not start recording other agents' conversations. An explicit stop requires stopping the active agent's capture and setting prompt recording to `stopped` when this new layer is in use.

Project evidence stays local. Explicit personal collection copies live in the configured reference library, defaulting to `~/.incline/library`. Explicit personal insight snapshots use the separate `~/.incline/personal-insights/` store (`--personal-dir`), and `--local-only` disables both stores. See [personal insight operations](insights.md#explicit-personal-insight-snapshots). Imported insight hypotheses live in `.incline/drafts/personal/` with selected portable evidence and a provenance receipt; they remain tentative until reviewed against the current project brief. Installation scope grants neither a data-sharing scope nor filesystem permissions. Do not commit, upload or promote evidence to the library without that user-requested scope. Project edits never modify a library snapshot or another project.
